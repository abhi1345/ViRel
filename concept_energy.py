...
def get_dataset(args, n_examples=None, isplot=False, is_load=False, is_rewrite=False, verbose=True, is_save_inter=False):
    ...
    dataset_subfilename = "/Datasets/{}-{}.p".format(args.dataset if not args.dataset.startswith("y-") else "c" + args.dataset[1:], dataset_2d_param)
    dataset_filename = REA_PATH + dataset_subfilename
    # args.dataset_subfilename = dataset_subfilename
...
if is_load_example_dataset:
    ...
    sys.path.append(os.path.join(os.path.dirname("__file__"), '..'))
    sys.path.append(os.path.join(os.path.dirname("__file__"), '..', '..'))
    from reasoning.experiments.concept_energy import get_dataset, ConceptDataset, ConceptClevrDataset
...
def rescale_data(pos_data, rescaled_size, rescale_mode="nearest"):
    ...
    if isinstance(pos_imgs, tuple):
        ...
        pos_imgs = (F.interpolate(pos_imgs[0], size=rescaled_size, mode=rescale_mode, antialias=True if rescale_mode=="bilinear" else False),
                    F.interpolate(pos_imgs[1], size=rescaled_size, mode=rescale_mode, antialias=True if rescale_mode=="bilinear" else False))
    else:
        pos_imgs = F.interpolate(pos_imgs, size=rescaled_size, mode=rescale_mode, antialias=True if rescale_mode=="bilinear" else False)
